...
function runSafeMode(input: SafeModeInput): void {
  ...
  const cleanQuotes = quotes
    .map(q => {
      ...
      const intrinsic = Math.max(0, (F - q.strike) / F);  // Normalize to fraction
  ...
  const results = cleanQuotes.map(q => {
    ...
    // Normalize to fraction: use F=1, K=strike/F
    const greeks = black76Greeks(1.0, q.strike / F, T, iv, true, 0);
