...
ws.on('message', (data) => {
  ...
  if (msg.params?.channel?.includes('ticker')) {
    ...
    if (d.mark_iv && d.mark_price && d.best_bid_price && d.best_ask_price) {
     ...
     // Feed data to ISM for continuous refitting (always do this after calibration)
     if (calibrated) {
    ...
    if (!calibrated && marketData.size >= 15) {
      ...
      if (range >= 40000) {
        ...
        // ✅ FILTER: Only use strikes with real liquidity
        const marketQuotes = Array.from(marketData.entries())
          .filter(([strike, data]) => {
            const minTick = 0.0001;
            return data.mid > 2 * minTick && 
                   data.iv > 0.15 && 
                   data.iv < 2.0 &&
                   data.bid > 0 &&
                   data.ask > 0;
        ...
        // ✅ Require at least 10 liquid quotes
