...
exports.IntegratedDualSurface = void 0;
exports.testIntegratedModel = testIntegratedModel;
const dualSurfaceModel_1 = require("./dualSurfaceModel");
const modelConfig_1 = require("./config/modelConfig");
const inventoryController_1 = require("./controllers/inventoryController");
const dualSurfaceModel_2 = require("./dualSurfaceModel");
const blackScholes_1 = require("./pricing/blackScholes");
class IntegratedDualSurface {
    surfaces;
    inventoryController;
    riskScorer;
    config;
    sviConfig; // Config format for SVI functions
    version;
    constructor(product = 'BTC') {
        ...
        this.inventoryController = new inventoryController_1.InventoryController(this.config);
        this.riskScorer = new dualSurfaceModel_2.RiskScorer();
        this.version = 0;
    }
    ...
    updateCC(expiry, metrics) {
        ...
        if (!surface) {
            ...
            surface.pcBumps = this.inventoryController.rebaseBumps(surface.cc, newCC, surface.pcBumps, expiry, spot);
            surface.cc = newCC;
        }
        this.version++;
    }
    ...
    onTrade(trade) {
        ...
        const variance = dualSurfaceModel_1.SVI.w(surface.cc, k);
        ...
        // 3. Update inventory
        const bucket = blackScholes_1.DeltaConventions.strikeToBucket(trade.strike, trade.spot, iv, trade.expiry);
        this.inventoryController.updateInventory(trade.strike, trade.size, greeks.vega, bucket);
    ...
    updateNodeState(surface, trade) {
        ...
        if (!node) {
            // Create new node
            const bucket = blackScholes_1.DeltaConventions.strikeToBucket(trade.strike, trade.spot, 0.3, // Approximate IV
            surface.expiry);
            node = {
                strike: trade.strike,
     ...
     */
    updatePCBumps(surface, expiry, spot) {
        ...
        for (const [bucket, bucketInv] of invState.byBucket) {
            if (Math.abs(bucketInv.signedVega) < 0.1)
                continue; // Skip if no significant inventory
            // Get strikes where we actually have positions in this bucket
            const strikesWithPositions = [];
            for (const [strike, node] of surface.nodes) {
                if (node.position !== 0) {
                    ...
                    const strikeBucket = blackScholes_1.DeltaConventions.strikeToBucket(strike, spot, iv, expiry);
            ...
            if (strikesWithPositions.length > 0) {
                const bumps = this.inventoryController.generateBumps(bucket, surface.cc, expiry, spot, strikesWithPositions);
                newBumps.push(...bumps);
            }
        }
        // Replace old bumps
     ...
     */
    getBucketStrikes(spot, expiry) {
        const bucketStrikes = new Map();
        // ATM strikes
    ...
    getQuote(expiry, strike, spot) {
        ...
        const ccVariance = dualSurfaceModel_1.SVI.w(surface.cc, k);
        const ccIV = Math.sqrt(ccVariance / expiry);
        const ccGreeks = (0, blackScholes_1.blackScholes)({
            strike,
            spot,
            vol: ccIV,
            T: expiry,
        ...
        const bumpAdjustment = dualSurfaceModel_1.BumpFunctions.evalBumps(surface.pcBumps, k);
        const pcVariance = ccVariance + bumpAdjustment;
        const pcIV = Math.sqrt(Math.max(pcVariance, 1e-8) / expiry);
        const pcGreeks = (0, blackScholes_1.blackScholes)({
            strike,
            spot,
            vol: pcIV,
            T: expiry,
        ...
        const bucket = blackScholes_1.DeltaConventions.strikeToBucket(strike, spot, ccIV, expiry);
        if (!node) {
            node = {
                strike,
        ...
        const pcMid = dualSurfaceModel_1.WidthDelta.getPCMid(node, currentWidth, ccMid, this.config.quotes.staleHours);
        // 5. Build quote
     ...
     */
    updateMarketObservations(observations) {
        this.riskScorer.updateFromMarket(observations);
    }
}
exports.IntegratedDualSurface = IntegratedDualSurface;
/**
 * Test the integrated model
 */
function testIntegratedModel() {
    console.log('\n' + '='.repeat(60));
    ...
    for (const [bucket, data] of Object.entries(inventory.byBucket)) {
        ...
        const edge = inventory.edges[bucket];
