 ...
 */
function testSurfaceCreation() {
    console.log('=== Test 1: Surface Creation ===\n');
    const config = (0, modelConfig_1.getDefaultConfig)('BTC');
    const model = new dualSurfaceModel_1.DualSurfaceModel(config);
    // Initial CC setup
    ...
    quotes.forEach((quote, strike) => {
        const bucket = blackScholes_1.DeltaConventions.strikeToBucket(strike, spot, 0.2, expiry);
...
function testWidthDeltaRule() {
    ...
    const initialDelta = Math.abs((0, blackScholes_1.blackScholes)({
        strike,
        spot,
        vol: 0.2,
        T: expiry,
    ...
    console.log(`  Bucket: ${blackScholes_1.DeltaConventions.strikeToBucket(strike, spot, 0.2, expiry)}\n`);
    // Execute trade
    ...
    const newDelta = Math.abs((0, blackScholes_1.blackScholes)({
        strike,
        spot,
        vol: 0.2,
        T: expiry,
    ...
    console.log(`New bucket: ${blackScholes_1.DeltaConventions.strikeToBucket(strike, spot, 0.2, expiry)}\n`);
...
function testGreeks() {
    ...
    const greeks = (0, blackScholes_1.blackScholes)(inputs);
    ...
    const callGreeks = (0, blackScholes_1.blackScholes)({ ...inputs, isCall: true });
    const putGreeks = (0, blackScholes_1.blackScholes)({ ...inputs, isCall: false });
