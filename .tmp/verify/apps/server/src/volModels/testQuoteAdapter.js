...
const quoteEngineAdapter_1 = require("./quoteEngineAdapter");
console.log('\n' + '='.repeat(60));
console.log('QUOTE ENGINE ADAPTER TEST');
console.log('='.repeat(60));
const adapter = new quoteEngineAdapter_1.QuoteEngineAdapter('BTC', 100);
// Register event handlers
adapter.on('fill', (fill) => {
    console.log(`\n✅ FILL: ${fill.side} ${fill.size}x ${fill.strike} @ ${fill.price.toFixed(2)}`);
});
adapter.on('risk', (risk) => {
    console.log(`\n📊 Risk Update:`);
