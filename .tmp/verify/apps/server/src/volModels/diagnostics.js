...
const integratedModel_1 = require("./integratedModel");
function diagnose() {
    console.log('\n=== DIAGNOSING PC BUMP ISSUE ===\n');
    const model = new integratedModel_1.IntegratedDualSurface('BTC');
    // Setup
    const expiry = 0.25;
    const spot = 100;
    const metrics = {
    ...
    for (const [bucket, edge] of Object.entries(inventory.edges)) {
        const bucketInv = inventory.byBucket[bucket];
        if (bucketInv) {
            console.log(`  ${bucket}: ${bucketInv.vega.toFixed(1)} vega → ${edge.toFixed(2)} ticks edge`);
        }
    }
    ...
    for (const k of testStrikes) {
        ...
        console.log(`${k.toString().padStart(6)} | ` +
            `${q.bucket.padEnd(6)} | ` +
