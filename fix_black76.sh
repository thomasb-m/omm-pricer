...
set -Eeuo pipefail
ROOT="$(pwd)"
B76="$ROOT/packages/vol-core/src/black76.ts"
IDX="$ROOT/packages/vol-core/src/index.ts"

mkdir -p "$(dirname "$B76")"

echo "🔧 Writing robust Black-76 implementation → $B76"
cat > "$B76" <<'TS'
...
# Ensure index.ts re-exports black76
echo "🔧 Ensuring index.ts re-exports black76 → $IDX"
mkdir -p "$(dirname "$IDX")"
if [ -f "$IDX" ]; then
  # keep existing exports, append missing export
  if ! grep -q "from './black76'" "$IDX"; then
    printf "\nexport * from './black76';\n" >> "$IDX"
    echo "✅ Appended export * from './black76' to index.ts"
  else
    echo "ℹ️  index.ts already exports from './black76'"
  fi
else
  cat > "$IDX" <<'TS'
export * from './units';
export * from './black76';
TS
  echo "✅ Created index.ts with units + black76 exports"
fi

echo "🏁 Done. Now run:"
echo "   npx vitest run apps/server/src/volModels/tests/black76_roundtrip.test.ts"
echo "   npx vitest run"
