export function convexRepair(strikes, tv, lower) {
    const n = tv.length;
    const x = tv.map((v, i) => Math.max(v, lower[i]));
    const widths = Array.from({ length: n - 1 }, (_, i) => Math.max(1e-12, strikes[i + 1] - strikes[i]));
    ...
    for (let i = 0; i < slopes.length; i++) {
        ...
        while (blocks.length >= 2) {
            ...
            if (a.sum / a.width <= b.sum / b.width)
                break;
...
export function projectThetaByCallConvexity(theta, strikes, forward, ccTV, taper, tol = 1e-6) {
    let shrinkCount = 0;
    let th = theta;
